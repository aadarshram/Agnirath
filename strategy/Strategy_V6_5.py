...
class Solar:
    ...
    def getincident(self, latitude, longitude, globaltime):
        ...
        a = res.to_pandas()
        # print(a.head())
        intensity = a.iloc[int(10+(globaltime//3600))]['ghi']
    ...
    def calculate_energy(self, time, globaltime, latitude, longitude):
        ...
        power = self.getincident(latitude, longitude, globaltime)*self.cell_efficiency*self.area
        energy = power*time
        return energy
...
def main():
    ...
    route_df = pd.read_csv("~/Workings/Agnirath_LVS_Strategy/Strategy/Strategy_V6/data.csv", delimiter=',')
